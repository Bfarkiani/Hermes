...
runTest() {
	 ...
	 ./run_catChunks_fixed_onl.sh $USERNAME ${NDN_DIR} ${NDNNAME_ONL} ${OUTPUT_FILE} ${RESULTS_FILE_ONL} ${PIPELINE_MAX} >& /tmp/run_catChunks_onl_direct.log 
	 sleep 5
	 echo "run catchunk arl"
	 ./run_catChunks_fixed_arl.sh $USERNAME ${NDN_DIR} ${NDNNAME_ARL} ${OUTPUT_FILE} ${RESULTS_FILE_ARL} ${PIPELINE_MAX} >& /tmp/run_catChunks_arl_direct.log 
	 sleep 5
     popd
...
sleep 20
d=$(date +%H-%M-%S)
RESULTS_FILE_ONL=${PWD}/RESULTS/${INPUT_FILE}_ONL_1mb_${d}_direct
RESULTS_FILE_ARL=${PWD}/RESULTS/${INPUT_FILE}_ARL_1mb_${d}_direct
